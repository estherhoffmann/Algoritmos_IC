#ifndef MULTIWAYMIXSOLVER_H
#define MULTIWAYMIXSOLVER_H
#include <iostream>
#include <vector>
#include <string>
#include <set>
...
private:
    ...
    // runs original multiway cut algorithm and returns terminal with expensivest cut
    int get_expensivest_cut(ListDigraph &, ListDigraph::ArcMap<int> &, ListDigraph::NodeMap<bool> &,
                    vector<int> &, vector<vector<tuple <int, int, int>>> &);

public:

	MultiwayMixSolver(int);
	~MultiwayMixSolver();
